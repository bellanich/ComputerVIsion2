...
for j = 1:size(selection, 1)
    ...
    RCol = cat(2, cat(1, Rm(j), [0, 0, 0]), [0; 0; 0; 1]);
    tCol = cat(1, tm(j), 0);

    next_cloud = (RCol * point_clouds(selection(2, j) + tCol))';
    merged_clouds = cat(3, merged_clouds, next_cloud); 
end
